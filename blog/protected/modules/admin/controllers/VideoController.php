...
class VideoController extends ControllerAdmin
{
	...
	public function actionCreate()
	{
		...
		foreach (Language::model()->getLanguage() as $key => $value) {
			$modelDesc[$value->code] = new VideoDescription;
		}
		...
		if(isset($_POST['Video']))
		{
			...
			foreach ($_POST['VideoDescription'] as $j => $mod) {
	            if (isset($_POST['VideoDescription'][$j])) {
	                $modelDesc[$j]=new VideoDescription; // if you had static model only
			...
			if($model->validate() AND $valid){
				...
				try
				{
					...
					foreach ($modelDesc as $key => $value) {
						$value->video_id=$model->id;
						$value->save();
					}

					Log::createLog("Video Controller Create $model->id");
	...
	public function actionUpdate($id)
	{
		...
		$modelDesc = array();
		foreach (Language::model()->getLanguage() as $key => $value) {
			$modelDesc[$value->code] = Video::model()->getDataDesc($model->id, $value->id);
			$modelDesc[$value->code] = ($modelDesc[$value->code]==null) ? new VideoDescription : $modelDesc[$value->code];
		...
		if(isset($_POST['Video']))
		{
			...
			$model->attributes=$_POST['Video'];//setting semua nilai
			...
			if($model->validate() AND $valid){
				...
				try
				{
					...
					VideoDescription::model()->deleteAll('video_id = :id', array(':id'=>$model->id));
					foreach ($modelDesc as $key => $value) {
						$value->video_id=$model->id;
						$value->save();
					}

					Log::createLog("VideoController Update $model->id");
